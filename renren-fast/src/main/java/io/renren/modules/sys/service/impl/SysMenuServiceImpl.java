...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuDao, SysMenuEntity> implements SysMenuService {
    ...
    public List<SysMenuEntity> queryListParentId(Long parentId, List<Long> menuIdList) {
        ...
        for (SysMenuEntity menu : menuList) {
            if (menuIdList.contains(menuList)) {
    ...
    private List<SysMenuEntity> getMenuList(List<Long> menuIdList) {
        ...
        List<SysMenuEntity> menus = this.baseMapper.selectList(
                new QueryWrapper<SysMenuEntity>().in(Objects.nonNull(menuIdList), "menu_id", menuIdList));
        // 将id和菜单绑定
        ...
        // 使用迭代器，组装菜单的层级关系
        Iterator<SysMenuEntity> iter = menus.iterator();
        while (iter.hasNext()) {
            SysMenuEntity menu = iter.next();
